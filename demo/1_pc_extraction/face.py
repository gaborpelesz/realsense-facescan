...
class FacePointCloud:
    ...
    def create_from_rgbd(self, rgb, d):
        ...
        self.ply_write_indexed('../scan_frames', pcd)

    def ply_write_indexed(self, path, pcd):
        with self.index_lock:
            new_index = self.index
            self.index += 1

        # write out ply file
        print(f"Writing '{new_index}.ply'...")
        o3d.io.write_point_cloud(os.path.join(path, f'{new_index}.ply'), pcd)
        print(f"Scan frame '{new_index}.ply' has been written!")
