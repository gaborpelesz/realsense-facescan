...
def bbox_dist_from_center(bbox, center):
    ...
    return np.linalg.norm(center - np.array([bbox[2] - bbox[0], bbox[3] - bbox[1]]))
...
def detect_face(img, scale_box=1):
    ...
    closest = bboxes[np.argmin(dists)]

    x_scaler = ((closest[2] - closest[0]) * scale_box - (closest[2] - closest[0])) / 2
    y_scaler = ((closest[3] - closest[1]) * scale_box - (closest[3] - closest[1])) / 2

    scaled = [(closest[0] - x_scaler, closest[1] - y_scaler), 
              (closest[2] + x_scaler, closest[3] + y_scaler)]

    return [(int(closest[0] - x_scaler), int(closest[1] - y_scaler)), 
            (int(closest[2] + x_scaler), int(closest[3] + y_scaler))]
